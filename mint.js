...
async function refreshContract() {
  ...
  $("mintModeBadge").textContent = state.mode === 0 ? "BNB" : `ERC20 (${state.mintPaySymbol})`;
...
async function mintNow() {
  ...
  if (mode === 0) {
    ...
    if (balance < price) throw new Error(`${state.mintPaySymbol} 余额不足，需要 ${formatAmount(price, state.mintPayDecimals)} ${state.mintPaySymbol}`);
