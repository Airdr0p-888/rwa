...
contract FairMintTokenV1 is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    function mintUSDT() external nonReentrant whenNotPaused { require(mintMode == MintMode.USDT, "not ERC20 mode"); uint256 beforeBalance = IERC20(usdtAddress).balanceOf(address(this)); IERC20(usdtAddress).safeTransferFrom(msg.sender, address(this), mintPrice); uint256 received = IERC20(usdtAddress).balanceOf(address(this)) - beforeBalance; require(received == mintPrice, "unsupported payment token"); _mintFlow(msg.sender, received); }
...
const state = { provider: null, signer: null, account: null, compiled: null, admin: null, mint: null, dividendAdmin: null, deployPaymentToken: null };
...
const parseToken = (v, decimals = 18) => ethers.parseUnits(String(v || "0"), decimals);
...
async function assertTokenBalance(tokenAddress, owner, amount, label, decimals = 18) {
  const token = new ethers.Contract(tokenAddress, ERC20_ABI, state.signer);
...
async function paymentInfo(contract) {
  const mode = Number(await contract.mintMode());
  if (mode === 0) {
    const defaults = activeNetworkDefaults();
    return { address: ZERO, symbol: defaults?.native || "BNB", decimals: 18, native: true };
  }
  const address = await contract.usdtAddress();
  const token = new ethers.Contract(address, ERC20_ABI, state.signer || state.provider);
  const [symbol, decimals] = await Promise.all([
    token.symbol().catch(() => "TOKEN"),
    token.decimals().catch(() => 18)
  ]);
  return { address, symbol, decimals: Number(decimals), native: false };
}
...
function deployPaymentMeta() {
  const mode = Number(formField("mintMode")?.value || 0);
  const defaults = activeNetworkDefaults();
  if (mode === 0) return { address: ZERO, symbol: defaults?.native || "BNB", decimals: 18, native: true };
  const address = String(formField("usdtAddress")?.value || "").trim();
  if (state.deployPaymentToken?.address?.toLowerCase() === address.toLowerCase()) return state.deployPaymentToken;
  return { address, symbol: "ERC20", decimals: 18, native: false };
}

async function refreshDeployPaymentToken() {
  const mode = Number(formField("mintMode")?.value || 0);
  const info = $("paymentTokenInfo");
  if (mode === 0) {
    state.deployPaymentToken = null;
    if (info) info.textContent = "（BNB 原生币）";
    updateDeployHints();
    return deployPaymentMeta();
  }
  const address = String(formField("usdtAddress")?.value || "").trim();
  if (!ethers.isAddress(address) || address === ZERO) {
    state.deployPaymentToken = null;
    if (info) info.textContent = "（请输入有效的 ERC20 合约地址）";
    updateDeployHints();
    return null;
  }
  if (!state.provider) {
    if (info) info.textContent = "（连接钱包后读取代币信息）";
    return null;
  }
  const token = new ethers.Contract(address, ERC20_ABI, state.provider);
  const [symbol, decimals] = await Promise.all([
    token.symbol().catch(() => "TOKEN"),
    token.decimals()
  ]);
  const parsedDecimals = Number(decimals);
  if (!Number.isInteger(parsedDecimals) || parsedDecimals < 0 || parsedDecimals > 36) throw new Error("支付代币 decimals 无效");
  state.deployPaymentToken = { address: ethers.getAddress(address), symbol, decimals: parsedDecimals, native: false };
  if (info) info.textContent = `（已识别 ${symbol}，${parsedDecimals} 位精度）`;
  updateDeployHints();
  return state.deployPaymentToken;
}
...
function launchPriceDetails(form) {
  const payment = deployPaymentMeta();
  const mintPriceRaw = parseToken(form.elements.mintPrice.value, payment.decimals);
...
function applyTargetLaunchPrice() {
  ...
  try {
    const payment = deployPaymentMeta();
    const targetPriceRaw = parseToken(rawValue, payment.decimals);
    const mintPriceRaw = parseToken(form.elements.mintPrice.value, payment.decimals);
    const userMintMode = Number(form.elements.userMintMode.value);
    const userShareBp = percentToBp(form.elements.userMintShare.value);
    const userMintAmountRaw = parseToken(form.elements.userMintAmount.value);
...
function updateDeployHints() {
  ...
  const defaults = activeNetworkDefaults();
  const currency = deployPaymentMeta().symbol;
  let launchPrice = "-";
  try {
...
function readDeployLimitConfig(form, baseDecimals = 18) {
  ...
  const config = {
    ...
    maxBaseAmount: parseToken(form.elements.maxBuyBaseAmountPerWallet.value, baseDecimals),
    ...
    timedAmounts: [
      parseToken(form.elements.timedBuyLimitAmount1.value, baseDecimals),
      parseToken(form.elements.timedBuyLimitAmount2.value, baseDecimals),
      parseToken(form.elements.timedBuyLimitAmount3.value, baseDecimals)
    ],
...
async function applyPostDeploySettings(contract, form) {
  ...
  const limit = readDeployLimitConfig(form, deployPaymentMeta().decimals);
  const jobs = [
...
async function deployArgs(form) {
  if (!(form instanceof HTMLFormElement)) throw new Error("没有读取到部署表单，请刷新页面后重试。");
  const fd = new FormData(form);
  const tax = readDeployTaxConfig(form);
  const mode = Number(fd.get("mintMode"));
  let payment = deployPaymentMeta();
  if (mode === 1) {
    payment = await refreshDeployPaymentToken();
    if (!payment) throw new Error("无法读取自定义支付代币，请检查代币地址、钱包网络和合约兼容性");
  }
  const limit = readDeployLimitConfig(form, payment.decimals);
  const modules = selectedModuleConfig();
  ...
  return [
    ...
    fd.get("router"),
    parseToken(fd.get("mintPrice"), payment.decimals),
...
async function deployContract(form) {
  ...
  setDefaultMarketingWallet();
  applyNetworkDefaults();
  const args = await deployArgs(form);
...
function saveDeployedToken(address, args) {
  try {
    ...
    existing.push({
      ...
      sym: symbol,
      color: color,
      price: Number(String(mintPrice === 0n ? '0' : ethers.formatUnits(mintPrice, deployPaymentMeta().decimals))),
      change: 0,
      cap: '--',
      vol: '--',
      progress: 0,
...
async function refreshMint() {
  ...
  const payment = await paymentInfo(state.mint);
  ...
  renderStats("mintStats", [
    ["Mint 价格", `${ethers.formatUnits(mintPrice, payment.decimals)} ${payment.symbol}`],
    ["单次代币", ethers.formatUnits(tokenPerMint, 18)],
    ["进度", `${mintedCount} / ${maxMintCount}`],
    ["Mint 状态", mintEnabled ? "开启" : "关闭"],
    ["模式", Number(mode) === 0 ? "BNB" : `ERC20 (${payment.symbol})`],
...
async function refreshAdmin() {
  ...
  const payment = await paymentInfo(state.admin);
  const [
...
async function mintNow() {
  ...
  if (mode === 0) await txDone(await state.mint.mintBNB({ value: price }), "Mint");
    ...
    const payment = await paymentInfo(state.mint);
    await assertTokenBalance(payment.address, state.account, price, `${payment.symbol} Mint`, payment.decimals);
    await approveIfNeeded(payment.address, contractAddress, price, `${payment.symbol} Mint`);
...
async function adminAction(action) {
  ...
  const payment = await paymentInfo(c);
  ...
  const calls = {
    setMintPrice: () => c.setMintPrice(parseToken($("newMintPrice").value, payment.decimals)),
    ...
    setMaxBuyBaseAmountPerWallet: () => c.setMaxBuyBaseAmountPerWallet(parseToken($("maxBuyBaseAmountPerWallet").value, payment.decimals)),
    setTimedBuyLimitEnabled: () => c.setTimedBuyLimitEnabled(parseBool($("timedBuyLimitEnabled").value)),
    setTimedBuyLimitTier1: () => c.setTimedBuyLimitTier(0, BigInt($("timedBuyLimitMinute1").value || 0), parseToken($("timedBuyLimitAmount1").value, payment.decimals)),
    setTimedBuyLimitTier2: () => c.setTimedBuyLimitTier(1, BigInt($("timedBuyLimitMinute2").value || 0), parseToken($("timedBuyLimitAmount2").value, payment.decimals)),
    setTimedBuyLimitTier3: () => c.setTimedBuyLimitTier(2, BigInt($("timedBuyLimitMinute3").value || 0), parseToken($("timedBuyLimitAmount3").value, payment.decimals)),
    ...
    forceAddLiquidity: async () => {
      ...
      const fundAmount = parseToken($("liqFundAmount").value, payment.decimals);
      if (mode === 0) return c.forceAddLiquidity(tokenAmount, fundAmount, { value: fundAmount });
      await approveIfNeeded(usdt, contractAddress, fundAmount, `${payment.symbol} 加池`);
...
formField("mintMode")?.addEventListener("change", () => {
  applyNetworkDefaults();
  refreshDeployPaymentToken().catch((error) => log(error.shortMessage || error.message || String(error)));
});
formField("usdtAddress")?.addEventListener("change", () => {
  state.deployPaymentToken = null;
  refreshDeployPaymentToken().catch((error) => log(error.shortMessage || error.message || String(error)));
});
