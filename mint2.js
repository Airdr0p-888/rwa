...
async function refreshContract() {
  ...
  state.mintPaySymbol = Number(mode) === 0 ? state.nativeSymbol : "TOKEN";
  ...
  if (Number(mode) === 1) {
    ...
    const payToken = new ethers.Contract(paymentAddress, ERC20_ABI, state.signer || state.provider);
    state.mintPaySymbol = await payToken.symbol().catch(() => "TOKEN");
  ...
  $("mintModeBadge").textContent = Number(mode) === 0 ? "BNB" : `ERC20 (${state.mintPaySymbol})`;
...
async function mintNow() {
  ...
  if (mode === 0) {
    ...
    if (balance < price) throw new Error(`${state.mintPaySymbol} balance is not enough.`);
    await approveIfNeeded(paymentAddress, state.contractAddress, price);
